  ...
  <div class="min-h-screen flex flex-col">
    ...
    <main class="flex-1 container mx-auto px-4 py-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- Card 1 -->
        <div class="bg-white shadow-md rounded-lg p-6  hover:bg-rose-300" id="card1" >
          <h3 class="text-black text-lg font-semibold flex justify-center">Nombre total d'utilisateurs <?= $_SESSION['totalUtilisateurs'] ?></h3>
        ...
        <div class="bg-white shadow-md rounded-lg p-6  hover:bg-rose-300" id="card3" onclick="">
          <h3 class="text-black text-lg font-semibold flex justify-center">Freelances inscrits <?= $_SESSION['totalFreelances'] ?></h3>
          <p class="text-2xl font-bold text-indigo-600 mt-2" ></p>
        </div>
        <!-- Card 4 -->
        <div class="bg-white shadow-md rounded-lg p-6  hover:bg-rose-300" id="card4" onclick="">
          <h3 class="text-black text-lg font-semibold flex justify-center">Offres en cours <?= $_SESSION['totalTemoignages'] ?></h3>
          <p class="text-2xl font-bold text-indigo-600 mt-2" ></p>
        </div>
      </div>
      <div class="bg-white shadow-md rounded-lg p-40 mt-4" id="affiche">
      <h1 class="text-2xl font-bold text-center text-black -mt-32">Liste des utilisateurs</h1>
    <table  class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <tbody class="text-gray-700 text-sm">
            <?php foreach ($_SESSION['allutilisateurs'] as $utilisateur): ?>
                <tr class="border-b border-rose-200 hover:bg-rose-100">
                    <td class="border-b border-rose-200 hover:bg-rose-100"><?= htmlspecialchars($utilisateur['id_utilisateur']) ?></td>
                    <td class="border-b border-rose-200 hover:bg-rose-100"><?= htmlspecialchars($utilisateur['nom_utilisateur']) ?></td>
                    <td class="border-b border-rose-200 hover:bg-rose-100"><?= htmlspecialchars($utilisateur['email']) ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <h1 class="text-2xl font-bold text-center text-gray-800 mb-6">Liste des Projets</h1>
